import type { MapTile, TerrainType } from '../types.js';
...
export class MapGenerator {
  generate(worldId: string, size: number): GeneratedMap {
    const tiles: MapTile[] = [];
  ...
  private buildNoise(size: number): number[][] {
    const grid: number[][] = [];
  ...
  private terrainFromNoise(n: number, x: number, y: number, size: number): TerrainType {
    ...
    if (adjusted < 0.15) return 'water';
    if (adjusted < 0.25) return 'swamp';
    if (adjusted < 0.45) return 'grassland';
    if (adjusted < 0.65) return 'forest';
    if (adjusted < 0.80) return 'grassland';
    return 'mountain';
  }

  private placeResourceNodes(
    worldId: string,
    tiles: MapTile[],
    size: number
  ): GeneratedMap['resourceNodes'] {
    const nodes: GeneratedMap['resourceNodes'] = [];
    ...
    for (const cfg of configs) {
      const candidates = passable.filter(t =>
        cfg.terrain_pref.length === 0 || cfg.terrain_pref.includes(t.terrain)
      );
