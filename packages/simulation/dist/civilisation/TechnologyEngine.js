...
exports.TechnologyEngine = void 0;
...
class TechnologyEngine {
     ...
     */
    async checkDiscoveries(agent, worldId, tick, day) {
        if (!agent.group_id)
            return [];
        const discovered = [];
        // Get all techs already known by this group
        const knownTechs = await (0, db_js_1.query)(`SELECT t.name FROM civilisation.technologies t
       JOIN civilisation.group_technologies gt ON gt.tech_id = t.tech_id
       WHERE gt.group_id = $1`, [agent.group_id]);
        const knownNames = new Set(knownTechs.map(t => t.name));
        // Get this agent's skills
        const skills = await (0, db_js_1.query)(`SELECT skill_name, level FROM agents.agent_skills WHERE agent_id = $1`, [agent.agent_id]);
        const skillMap = new Map(skills.map(s => [s.skill_name, s.level]));
        for (const techDef of TECH_TREE) {
            if (knownNames.has(techDef.name))
                continue;
            ...
            if (qualifies) {
                ...
                if (tech)
                    discovered.push(tech);
            }
        }
        return discovered;
    }
    async discoverTech(agent, techDef, worldId, tick, day) {
        const tech = await (0, db_js_1.queryOne)(`INSERT INTO civilisation.technologies
       ...
       RETURNING *`, [
        ...
        if (agent.group_id) {
         ...
         VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [agent.group_id, tech.tech_id, tick]);
        }
        // Add to agent's known_tech_ids
        await (0, db_js_1.execute)(`UPDATE agents.agents SET known_tech_ids = array_append(known_tech_ids, $2::uuid)
       WHERE agent_id = $1`, [agent.agent_id, tech.tech_id]);
        return tech;
    }
    // ── Tech spreading ────────────────────────────────────────────────────────────
    /**
     ...
     */
    async spreadTech(sourceTechId, sourceGroupId, targetGroupId, worldId, tick) {
        // Check alliance treaty
        const treaty = await (0, db_js_1.queryOne)(`SELECT treaty_id FROM conflict.treaties
       WHERE ((group_a_id = $1 AND group_b_id = $2) OR (group_a_id = $2 AND group_b_id = $1))
         AND treaty_type IN ('alliance', 'resource_sharing')
         AND status = 'active'`, [sourceGroupId, targetGroupId]);
        if (!treaty)
            return false;
        // Add tech to target group
        await (0, db_js_1.execute)(`INSERT INTO civilisation.group_technologies (group_id, tech_id, adopted_tick)
       VALUES ($1, $2, $3) ON CONFLICT DO NOTHING`, [targetGroupId, sourceTechId, tick]);
        return true;
    }
    // ── Tech loss ─────────────────────────────────────────────────────────────────
    /**
     ...
     */
    async checkTechLoss(deadAgentId, groupId, worldId) {
        if (!groupId)
            return [];
        const lost = [];
        // Find techs discovered by this agent
        const agentTechs = await (0, db_js_1.query)(`SELECT t.* FROM civilisation.technologies t
       WHERE t.discovered_by = $1 AND t.is_lost = FALSE`, [deadAgentId]);
        for (const tech of agentTechs) {
            // Check if any other group member has this tech (knows it)
            const otherKnowers = await (0, db_js_1.queryOne)(`SELECT COUNT(*) as count
         FROM agents.agents
         WHERE group_id = $1
           AND status = 'alive'
           AND agent_id != $2
           AND $3 = ANY(known_tech_ids)`, [groupId, deadAgentId, tech.tech_id]);
            if (!otherKnowers || otherKnowers.count === 0) {
                // Tech is lost — small chance recovery (5%)
                if (Math.random() > 0.05) {
                    await (0, db_js_1.execute)(`UPDATE civilisation.technologies SET is_lost = TRUE WHERE tech_id = $1`, [tech.tech_id]);
                    lost.push(tech);
                }
            }
        }
    ...
    // ── Query helpers ─────────────────────────────────────────────────────────────
    async getGroupTechs(groupId) {
        return (0, db_js_1.query)(`SELECT t.* FROM civilisation.technologies t
       JOIN civilisation.group_technologies gt ON gt.tech_id = t.tech_id
       WHERE gt.group_id = $1 AND t.is_lost = FALSE`, [groupId]);
    }
    async getUnlockedActions(groupId) {
    ...
    async getAvailableTechs(groupId, worldId) {
        const knownTechs = await (0, db_js_1.query)(`SELECT t.name FROM civilisation.technologies t
       JOIN civilisation.group_technologies gt ON gt.tech_id = t.tech_id
       WHERE gt.group_id = $1`, [groupId]);
        const knownNames = new Set(knownTechs.map(t => t.name));
        // Get all group member skills (max level per skill across all members)
        const groupSkills = await (0, db_js_1.query)(`SELECT skill_name, MAX(level) as level
       FROM agents.agent_skills
       WHERE agent_id IN (
         SELECT agent_id FROM social.group_members WHERE group_id = $1
       )
       GROUP BY skill_name`, [groupId]);
    ...
    async getWorldTechs(worldId) {
        return (0, db_js_1.query)(`SELECT * FROM civilisation.technologies WHERE world_id = $1 AND is_lost = FALSE
       ORDER BY tech_tier, discovered_tick`, [worldId]);
    }
}
exports.TechnologyEngine = TechnologyEngine;
