...
const POSITIVE_CLAIMS = ['trustworthy', 'skilled', 'generous', 'heroic'];
const NEGATIVE_CLAIMS = ['dangerous', 'deceptive', 'weak', 'cruel'];
/**
...
class GossipEngine {
     ...
     */
    async processGossip(gossiper, listener, subject, claim, tick, day) {
        ...
        const row = await (0, db_js_1.queryOne)(`INSERT INTO social.gossip_events
     ...
     */
    async getReputation(agentId, worldId) {
        const rep = await (0, db_js_1.queryOne)(`SELECT * FROM social.reputation WHERE agent_id = $1 AND world_id = $2`, [agentId, worldId]);
        if (rep)
            return rep;
        // Bootstrap
        await (0, db_js_1.execute)(`INSERT INTO social.reputation (agent_id, world_id)
       VALUES ($1, $2)
       ON CONFLICT DO NOTHING`, [agentId, worldId]);
        return {
            agent_id: agentId,
            world_id: worldId,
            trustworthiness: 50,
     ...
     */
    async getReputationSummary(agentId, worldId) {
        const rep = await this.getReputation(agentId, worldId);
        if (rep.total_reports === 0)
            return 'Unknown to most';
        const parts = [];
        if (rep.trustworthiness >= 70)
            parts.push('widely trusted');
        else if (rep.trustworthiness <= 30)
            parts.push('considered untrustworthy');
        if (rep.skill_renown >= 60)
            parts.push('respected for skill');
        if (rep.danger_level >= 60)
            parts.push('feared by many');
        if (rep.generosity >= 70)
            parts.push('known for generosity');
     ...
     */
    async boostReputation(agentId, worldId, dimension, amount, tick) {
        await (0, db_js_1.execute)(`INSERT INTO social.reputation (agent_id, world_id, ${dimension}, total_reports, positive_reports, last_updated_tick)
       ...
       ON CONFLICT (agent_id, world_id) DO UPDATE SET
         ...
         last_updated_tick = $4`, [agentId, worldId, amount, tick]);
    }
    // ── Private helpers ─────────────────────────────────────────────────────────
    /**
     ...
     * Range 0.2–0.9.
     */
    computeCredibility(gossiperTrust, listenerScepticism) {
        let base = 0.5;
    ...
    async updateReputation(subjectId, worldId, claim, tick) {
        const isPositive = POSITIVE_CLAIMS.includes(claim);
        ...
        await (0, db_js_1.execute)(`INSERT INTO social.reputation (agent_id, world_id, ${dimension}, total_reports, positive_reports, negative_reports, last_updated_tick)
    ...
    async shiftRelationship(agentId, otherId, delta) {
    ...
    async getRelationship(agentId, otherId) {
        return (0, db_js_1.queryOne)(`SELECT trust_score FROM social.relationships
       WHERE agent_id = $1 AND other_agent_id = $2`, [agentId, otherId]);
    }
}
exports.GossipEngine = GossipEngine;
