...
class GroupEngine {
    // ── Formation ───────────────────────────────────────────────────────────────
    async formGroup(founder, name, purpose, tick, day) {
        ...
        const row = await (0, db_js_1.queryOne)(`INSERT INTO social.groups
       ...
       RETURNING group_id`, [
        ...
        await (0, db_js_1.execute)(`INSERT INTO social.group_members
         (group_id, agent_id, role, joined_tick, joined_day, contribution_score)
       VALUES ($1, $2, 'founder', $3, $4, 10)`, [groupId, founder.agent_id, tick, day]);
        // Mark agent's current group
        await (0, db_js_1.execute)(`UPDATE agents.agents SET group_id = $1 WHERE agent_id = $2`, [groupId, founder.agent_id]);
        return {
            group_id: groupId,
    ...
    // ── Joining ─────────────────────────────────────────────────────────────────
    async joinGroup(agent, groupName, tick, day) {
        // Find group by name in this world
        const group = await (0, db_js_1.queryOne)(`SELECT * FROM social.groups
       WHERE world_id = $1 AND LOWER(name) = LOWER($2) AND status = 'active'`, [agent.world_id, groupName]);
        if (!group || !group.group_id)
            return null;
        // Already a member?
        const existing = await (0, db_js_1.queryOne)(`SELECT group_id FROM social.group_members
       WHERE group_id = $1 AND agent_id = $2`, [group.group_id, agent.agent_id]);
        if (existing)
            return group;
        ...
        await (0, db_js_1.execute)(`INSERT INTO social.group_members
         (group_id, agent_id, role, joined_tick, joined_day, contribution_score)
       VALUES ($1, $2, 'recruit', $3, $4, 0)`, [group.group_id, agent.agent_id, tick, day]);
        await (0, db_js_1.execute)(`UPDATE agents.agents SET group_id = $1 WHERE agent_id = $2`, [group.group_id, agent.agent_id]);
        await (0, db_js_1.execute)(`UPDATE social.groups SET member_count = member_count + 1 WHERE group_id = $1`, [group.group_id]);
        return group;
    }
    // ── Leaving / Disbanding ────────────────────────────────────────────────────
    async leaveGroup(agent, tick) {
        if (!agent.group_id)
            return;
        await (0, db_js_1.execute)(`DELETE FROM social.group_members WHERE group_id = $1 AND agent_id = $2`, [agent.group_id, agent.agent_id]);
        await (0, db_js_1.execute)(`UPDATE agents.agents SET group_id = NULL WHERE agent_id = $1`, [agent.agent_id]);
        await (0, db_js_1.execute)(`UPDATE social.groups
       SET member_count = GREATEST(0, member_count - 1)
       WHERE group_id = $1`, [agent.group_id]);
        // Check if group should disband (0 members left)
        const grp = await (0, db_js_1.queryOne)(`SELECT member_count FROM social.groups WHERE group_id = $1`, [agent.group_id]);
        if (grp && grp.member_count === 0) {
            await (0, db_js_1.execute)(`UPDATE social.groups SET status = 'disbanded', disbanded_tick = $1 WHERE group_id = $2`, [tick, agent.group_id]);
        }
    }
    // ── Per-tick pass ────────────────────────────────────────────────────────────
    /**
     ...
     */
    async runGroupTick(worldId, tick) {
        // Apply belonging bonus to all group members
        await (0, db_js_1.execute)(`UPDATE agents.agent_state s
       SET need_belonging = LEAST(100, need_belonging + $1)
       FROM agents.agents a
       WHERE s.agent_id = a.agent_id
         AND a.world_id = $2
         AND a.group_id IS NOT NULL
         AND a.status = 'alive'`, [GROUP_BELONGING_BONUS, worldId]);
        // Daily check: decay contribution scores + check disband
        if (tick % 1440 === 0 && tick > 0) {
            await this.dailyCohesionCheck(worldId, tick);
        }
    }
    async dailyCohesionCheck(worldId, tick) {
        // Get all active groups in this world
        const groups = await (0, db_js_1.query)(`SELECT group_id, name FROM social.groups WHERE world_id = $1 AND status = 'active'`, [worldId]);
        for (const grp of groups) {
            // Decay contribution scores of members who haven't acted recently
            await (0, db_js_1.execute)(`UPDATE social.group_members
         SET contribution_score = contribution_score - $1
         WHERE group_id = $2`, [INACTIVITY_CONTRIBUTION_DECAY * 1440, grp.group_id]);
            // Check average contribution — if too low, disband
            const stats = await (0, db_js_1.queryOne)(`SELECT AVG(contribution_score) as avg_contribution, COUNT(*) as member_count
         FROM social.group_members
         WHERE group_id = $1`, [grp.group_id]);
            if (stats && stats.avg_contribution < DISBAND_THRESHOLD) {
                await this.disbandGroup(grp.group_id, tick);
            }
        }
    }
    async disbandGroup(groupId, tick) {
        // Clear group_id from all members
        await (0, db_js_1.execute)(`UPDATE agents.agents SET group_id = NULL
       WHERE group_id = $1`, [groupId]);
        // Mark disbanded
        await (0, db_js_1.execute)(`UPDATE social.groups SET status = 'disbanded', disbanded_tick = $1 WHERE group_id = $2`, [tick, groupId]);
    }
     ...
     */
    async recordContribution(agentId, groupId, amount) {
        await (0, db_js_1.execute)(`UPDATE social.group_members
       SET contribution_score = contribution_score + $1
       WHERE group_id = $2 AND agent_id = $3`, [amount, groupId, agentId]);
    }
    // ── Query helpers ────────────────────────────────────────────────────────────
    async getGroup(groupId) {
        return (0, db_js_1.queryOne)(`SELECT * FROM social.groups WHERE group_id = $1`, [groupId]);
    }
    async getGroupMembers(groupId) {
        return (0, db_js_1.query)(`SELECT gm.*, a.name
       FROM social.group_members gm
       JOIN agents.agents a ON a.agent_id = gm.agent_id
       WHERE gm.group_id = $1
       ORDER BY gm.contribution_score DESC`, [groupId]);
    }
    async getAgentGroup(agentId) {
        return (0, db_js_1.queryOne)(`SELECT g.* FROM social.groups g
       JOIN agents.agents a ON a.group_id = g.group_id
       WHERE a.agent_id = $1 AND g.status = 'active'`, [agentId]);
    }
    async getWorldGroups(worldId) {
        return (0, db_js_1.query)(`SELECT * FROM social.groups
       WHERE world_id = $1 AND status = 'active'
       ORDER BY member_count DESC`, [worldId]);
    }
    async getNearbyGroups(worldId, x, y, radius) {
        return (0, db_js_1.query)(`SELECT * FROM social.groups
       WHERE world_id = $1 AND status = 'active'
    ...
    async pickColour(worldId) {
        const usedColours = await (0, db_js_1.query)(`SELECT colour FROM social.groups WHERE world_id = $1 AND status = 'active'`, [worldId]);
