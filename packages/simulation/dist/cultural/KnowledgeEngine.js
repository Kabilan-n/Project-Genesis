...
class KnowledgeEngine {
     ...
     */
    async attemptSkillTeaching(agentA, agentB, conversation) {
        ...
        for (const skillA of agentA.skills) {
            ...
            if (levelGap < 5)
                continue; // Only teach if gap is meaningful
            const xpTransfer = levelGap * XP_TRANSFER_RATIO;
            const teaching = await this.transferSkill(agentA, agentB, skillA.skill_name, xpTransfer, skillA.level, skillB.level, conversation.conversation_id, conversation.tick, conversation.day);
            if (teaching)
        ...
        for (const skillB of agentB.skills) {
            ...
            if (levelGap < 5)
                continue;
            const xpTransfer = levelGap * XP_TRANSFER_RATIO;
            const teaching = await this.transferSkill(agentB, agentA, skillB.skill_name, xpTransfer, skillB.level, skillA.level, conversation.conversation_id, conversation.tick, conversation.day);
            if (teaching)
                teachings.push(teaching);
        }
        return teachings;
    }
    async transferSkill(teacher, student, skillName, xpAmount, teacherLevel, studentLevelBefore, conversationId, tick, day) {
        // Add XP to student
        await (0, db_js_1.execute)(`UPDATE agents.agent_skills
       SET xp = xp + $1
       WHERE agent_id = $2 AND skill_name = $3`, [xpAmount, student.agent_id, skillName]);
        // Level up if XP threshold reached (100 XP per level)
        const newSkillRow = await (0, db_js_1.queryOne)(`SELECT level, xp FROM agents.agent_skills WHERE agent_id = $1 AND skill_name = $2`, [student.agent_id, skillName]);
        ...
        if (newSkillRow && newSkillRow.xp >= newSkillRow.level * 100) {
            ...
            await (0, db_js_1.execute)(`UPDATE agents.agent_skills SET level = $1, xp = 0 WHERE agent_id = $2 AND skill_name = $3`, [newLevel, student.agent_id, skillName]);
        }
        // Record the teaching event
        const row = await (0, db_js_1.queryOne)(`INSERT INTO agents.skill_teachings
     ...
     */
    async propagateKnowledge(agentA, agentB, conversation) {
        if (conversation.outcome !== 'bonding' && conversation.outcome !== 'friendly') {
            return;
        }
        // Share from A to B
    ...
    async shareFacts(sharer, receiver) {
        // Get sharer's top-confidence facts not already known by receiver
        const sharerFacts = await (0, db_js_1.query)(`SELECT k.* FROM memory.agent_knowledge k
       WHERE k.agent_id = $1
       ...
       LIMIT 3`, [sharer.agent_id, receiver.agent_id]);
        for (const fact of sharerFacts) {
            ...
            await (0, db_js_1.execute)(`UPDATE memory.agent_knowledge SET times_shared = times_shared + 1
         WHERE agent_id = $1 AND fact_key = $2`, [sharer.agent_id, fact.fact_key]);
        }
    }
    /**
     * Record a new fact an agent has directly discovered (e.g., found a resource node).
     */
    async recordDiscovery(agentId, worldId, factKey, factValue, tick) {
        await this.upsertKnowledge({
            agent_id: agentId,
            world_id: worldId,
            fact_key: factKey,
        ...
        await (0, db_js_1.execute)(`DELETE FROM memory.agent_knowledge
       WHERE agent_id = $1
         AND knowledge_id IN (
    ...
    async getKnowledge(agentId) {
        return (0, db_js_1.query)(`SELECT * FROM memory.agent_knowledge
       WHERE agent_id = $1
       ORDER BY confidence DESC, times_shared DESC
       LIMIT 10`, [agentId]);
    }
    async upsertKnowledge(k) {
        await (0, db_js_1.execute)(`INSERT INTO memory.agent_knowledge
