...
const ConversationEngine_js_1 = require("../social/ConversationEngine.js");
const RelationshipEngine_js_1 = require("../social/RelationshipEngine.js");
const TradeEngine_js_1 = require("../social/TradeEngine.js");
const GossipEngine_js_1 = require("../cultural/GossipEngine.js");
...
const GovernanceEngine_js_1 = require("../conflict/GovernanceEngine.js");
...
const HP_DAMAGE = {
    starvation: 10 / 1440, // food < 5
    dehydration: 15 / 1440, // water < 5
    exhaustion: 5 / 1440, // rest < 5
};
...
class AgentEngine {
    ...
    governanceEngine;
    beliefEngine;
    constructor(worldEngine, redis) {
        this.worldEngine = worldEngine;
        this.redis = redis;
        this.llm = LLMFactory_js_1.LLMFactory.fromEnv();
        this.promptBuilder = new PromptBuilder_js_1.PromptBuilder();
        this.conversationEngine = new ConversationEngine_js_1.ConversationEngine();
        this.relationshipEngine = new RelationshipEngine_js_1.RelationshipEngine();
        this.tradeEngine = new TradeEngine_js_1.TradeEngine();
        this.gossipEngine = new GossipEngine_js_1.GossipEngine();
        this.groupEngine = new GroupEngine_js_1.GroupEngine();
        this.knowledgeEngine = new KnowledgeEngine_js_1.KnowledgeEngine();
        this.conflictEngine = new ConflictEngine_js_1.ConflictEngine();
        this.governanceEngine = new GovernanceEngine_js_1.GovernanceEngine();
        this.beliefEngine = new BeliefEngine_js_1.BeliefEngine();
    }
    async runAgentTick(agentId, tick, day) {
        ...
        // 7. DECIDE via LLM
        const prompt = this.promptBuilder.buildDecisionPrompt({ ...agent, state: updatedState }, relationships, memories, perception, tick, day, knowledgeFacts, agentGroup, reputationSummary, nearbyGroups);
    ...
    updateNeeds(state, _tick) {
        ...
        // HP damage from critical needs
        let hpChange = 0;
        if (s.need_food < 5)
            hpChange -= HP_DAMAGE.starvation * 100;
        if (s.need_water < 5)
            hpChange -= HP_DAMAGE.dehydration * 100;
        if (s.need_rest < 5 && s.is_awake)
            hpChange -= HP_DAMAGE.exhaustion * 100;
    ...
    computeMentalState(s) {
        if (s.need_food < 20 || s.need_water < 20)
            return 'desperate';
        if (s.need_rest < 20 && s.is_awake)
            return 'tired';
        if (s.need_food < 40 || s.need_water < 40)
            return 'stressed';
        if (s.need_belonging < 25)
            return 'depressed';
        return 'content';
    }
    async perceive(agent, state) {
        const RADIUS = 6;
        // Nearby agents
        const nearbyAgentRows = await (0, db_js_1.query)(`SELECT a.agent_id, a.name, s.position_x, s.position_y, s.current_activity
       ...
       WHERE a.world_id = $1 AND a.status = 'alive' AND a.agent_id != $2
         AND ABS(s.position_x - $3) <= $4 AND ABS(s.position_y - $5) <= $4`, [agent.world_id, agent.agent_id, state.position_x, RADIUS, state.position_y]);
        // Relationships for nearby agents
        const nearbyIds = nearbyAgentRows.map(r => r.agent_id);
        const relRows = nearbyIds.length > 0
            ? await (0, db_js_1.query)(`SELECT other_agent_id, relationship_type FROM social.relationships
           WHERE agent_id = $1 AND other_agent_id = ANY($2)`, [agent.agent_id, nearbyIds])
            : [];
        ...
        // Visible resources
        const resourceRows = await (0, db_js_1.query)(`SELECT resource_type, current_amount, x, y
       FROM worlds.resource_nodes
       WHERE world_id = $1 AND NOT is_depleted
         AND ABS(x - $2) <= $3 AND ABS(y - $4) <= $3
       ORDER BY ABS(x - $2) + ABS(y - $4) ASC
       LIMIT 10`, [agent.world_id, state.position_x, RADIUS, state.position_y]);
        ...
        // Current tile
        const tileRow = await (0, db_js_1.queryOne)(`SELECT terrain, is_passable FROM worlds.map_tiles
       WHERE world_id = $1 AND x = $2 AND y = $3`, [agent.world_id, state.position_x, state.position_y]);
        ...
        for (const d of dirs) {
            ...
            if (nx < 0 || nx >= WORLD_SIZE || ny < 0 || ny >= WORLD_SIZE)
                continue;
            const tile = await (0, db_js_1.queryOne)('SELECT is_passable FROM worlds.map_tiles WHERE world_id = $1 AND x = $2 AND y = $3', [agent.world_id, nx, ny]);
            if (tile?.is_passable)
                passable_directions.push(d.name);
        }
        ...
        if (agent.group_id) {
            ...
            if (activeWars.length > 0) {
                ...
                const enemyGroupIds = activeWars.map(w => w.aggressor_group_id === agent.group_id ? w.defender_group_id : w.aggressor_group_id);
                const enemyGroups = await (0, db_js_1.query)(`SELECT group_id, name FROM social.groups WHERE group_id = ANY($1)`, [enemyGroupIds]);
                ...
                const nearbyWithGroup = nearbyIds.length > 0
                    ? await (0, db_js_1.query)(`SELECT agent_id, group_id FROM agents.agents WHERE agent_id = ANY($1)`, [nearbyIds])
                    : [];
                ...
                conflict_context = {
                    active_wars: activeWars.map(w => ({
                        ...
                        enemy_group_name: enemyGroupMap.get(w.aggressor_group_id === agent.group_id ? w.defender_group_id : w.aggressor_group_id) ?? 'unknown',
        ...
        const nearbySacredSites = await (0, db_js_1.query)(`SELECT ss.site_id, ss.name, bs.name AS belief_name, ss.x, ss.y
       FROM culture.sacred_sites ss
       JOIN culture.belief_systems bs ON bs.belief_id = ss.belief_id
       WHERE ss.world_id = $1
         AND ABS(ss.x - $2) <= 8 AND ABS(ss.y - $3) <= 8`, [agent.world_id, state.position_x, state.position_y]);
        // Nearby preachers: agents with a belief that are nearby and active
        const nearbyPreachers = [];
        ...
        return {
            ...
            current_tile: {
                ...
                x: state.position_x,
                y: state.position_y,
                terrain: tileRow?.terrain ?? 'grassland',
    ...
    async executeAction(agent, state, decision, tick, day, perception) {
        const s = { ...state };
        ...
        switch (verb) {
            ...
            case 'gather': {
                ...
                const node = await (0, db_js_1.queryOne)(`SELECT node_id, current_amount, extraction_rate FROM worlds.resource_nodes
           WHERE world_id = $1 AND x = $2 AND y = $3 AND resource_type = $4 AND NOT is_depleted`, [agent.world_id, s.position_x, s.position_y, resourceType]);
                if (node) {
            ...
            case 'eat': {
                ...
                const inv = await (0, db_js_1.queryOne)(`SELECT amount FROM economy.inventory WHERE agent_id = $1 AND resource_type = 'food'`, [agent.agent_id]);
                if (inv && inv.amount > 0) {
                    const eaten = Math.min(amount, inv.amount);
                    await (0, db_js_1.execute)(`UPDATE economy.inventory SET amount = amount - $1 WHERE agent_id = $2 AND resource_type = 'food'`, [eaten, agent.agent_id]);
            ...
            case 'drink': {
                const amount = Math.min(parseFloat(parts[1] ?? '10'), 50);
                const inv = await (0, db_js_1.queryOne)(`SELECT amount FROM economy.inventory WHERE agent_id = $1 AND resource_type = 'water'`, [agent.agent_id]);
                if (inv && inv.amount > 0) {
                    const drunk = Math.min(amount, inv.amount);
                    await (0, db_js_1.execute)(`UPDATE economy.inventory SET amount = amount - $1 WHERE agent_id = $2 AND resource_type = 'water'`, [drunk, agent.agent_id]);
            ...
            case 'talk': {
                ...
                const targetInfo = perception.nearby_agents.find(a => a.name.toLowerCase() === targetName?.toLowerCase());
                if (targetInfo && openingMessage) {
                    const targetAgent = await this.loadAgent(targetInfo.agent_id);
                    if (targetAgent) {
                        const conv = await this.conversationEngine.runConversation({ ...agent, state: s }, targetAgent, openingMessage, tick, day);
                        ...
                        if (conv.conversation_id && eventId) {
                            await (0, db_js_1.execute)(`UPDATE social.conversations SET event_id = $1 WHERE conversation_id = $2`, [eventId, conv.conversation_id]);
                        }
            ...
            case 'offer_trade': {
                ...
                if (targetInfo && decision.trade_offer) {
                    const targetAgent = await this.loadAgent(targetInfo.agent_id);
                    if (targetAgent) {
                        const trade = await this.tradeEngine.executeTrade({ ...agent, state: s }, targetAgent, decision.trade_offer, tick, day);
                        ...
                        if (trade.trade_id && eventId) {
                            await (0, db_js_1.execute)(`UPDATE economy.trades SET event_id = $1 WHERE trade_id = $2`, [eventId, trade.trade_id]);
                        }
                    }
                }
                break;
            ...
            case 'gossip': {
                ...
                const claim = (decision.gossip_claim ?? parts[3]);
                const listenerInfo = perception.nearby_agents.find(a => a.name.toLowerCase() === listenerName?.toLowerCase());
                if (listenerInfo && subjectName && claim) {
                    // Resolve subject agent — they don't need to be nearby
                    const subjectRow = await (0, db_js_1.queryOne)(`SELECT agent_id FROM agents.agents
             WHERE world_id = $1 AND LOWER(name) = LOWER($2) AND status = 'alive'`, [agent.world_id, subjectName]);
                    if (subjectRow) {
                        ...
                        if (listenerAgent && subjectAgent) {
                            const gossipEvent = await this.gossipEngine.processGossip({ ...agent, state: s }, listenerAgent, subjectAgent, claim, tick, day);
            ...
            case 'form_group': {
                ...
                if (groupName) {
                    const newGroup = await this.groupEngine.formGroup({ ...agent, state: s }, groupName, groupPurpose, tick, day);
            ...
            case 'join_group': {
                ...
                if (targetGroupName) {
                    const joined = await this.groupEngine.joinGroup({ ...agent, state: s }, targetGroupName, tick, day);
                    if (joined) {
            ...
            case 'declare_war': {
                ...
                const targetGroup = await (0, db_js_1.queryOne)(`SELECT group_id, name FROM social.groups WHERE world_id = $1 AND LOWER(name) = LOWER($2)`, [agent.world_id, targetGroupName]);
                if (targetGroup) {
                    const war = await this.conflictEngine.declareWar(agent.group_id, targetGroup.group_id, agent.world_id, tick, day);
                    if (war) {
                ...
                if (targetInfo) {
                    ...
                    if (targetAgent) {
                        const activeWar = agent.group_id && targetAgent.group_id
                            ...
                            : null;
                        const skirmish = await this.conflictEngine.executeRaid({ ...agent, state: s }, targetAgent, agent.world_id, tick, day, activeWar?.war_id);
            ...
            case 'propose_treaty': {
                ...
                const treatyType = (decision.treaty_type ?? parts[1] ?? 'non_aggression');
                const tGroup = await (0, db_js_1.queryOne)(`SELECT group_id, name FROM social.groups WHERE world_id = $1 AND LOWER(name) = LOWER($2)`, [agent.world_id, treatyTargetName]);
                if (tGroup) {
                    const treaty = await this.conflictEngine.proposeTreaty(agent.group_id, tGroup.group_id, treatyType, agent.world_id, tick, day);
                    if (treaty) {
            ...
            case 'challenge_leader': {
                if (!agent.group_id)
                    break;
                const event = await this.governanceEngine.challengeLeadership({ ...agent, state: s }, agent.group_id, agent.world_id, tick, day);
                if (event) {
            ...
            case 'exile_member': {
                ...
                const exileRow = await (0, db_js_1.queryOne)(`SELECT agent_id, name FROM agents.agents WHERE world_id = $1 AND LOWER(name) = LOWER($2) AND status = 'alive'`, [agent.world_id, exileTargetName]);
                if (exileRow) {
                    const exiled = await this.governanceEngine.exileMember(agent.agent_id, exileRow.agent_id, agent.group_id, agent.world_id);
                    if (exiled) {
            ...
            case 'defect_to': {
                ...
                const defectGroup = await (0, db_js_1.queryOne)(`SELECT group_id, name FROM social.groups WHERE world_id = $1 AND LOWER(name) = LOWER($2)`, [agent.world_id, defectGroupName]);
                if (defectGroup) {
                    const defected = await this.governanceEngine.defectToGroup({ ...agent, state: s }, defectGroup.group_id, tick, day);
                    if (defected) {
            ...
            case 'found_belief': {
                ...
                const belief = await this.beliefEngine.foundBelief({ ...agent, state: s }, bName, bTenet, tick, day);
                if (belief) {
            ...
            case 'preach': {
                ...
                const preachTargetInfo = perception.nearby_agents.find(a => a.name.toLowerCase() === preachTargetName?.toLowerCase());
                if (preachTargetInfo) {
                    const result = await this.beliefEngine.preach({ ...agent, state: s }, preachTargetInfo.agent_id, agent.world_id, tick, day);
            ...
            case 'record_myth': {
                ...
                const myth = await this.beliefEngine.recordMyth({ ...agent, state: s }, null, mTitle, mNarrative, agent.world_id, tick, day);
                if (myth) {
            ...
            case 'claim_sacred': {
                ...
                const site = await this.beliefEngine.claimSacredSite({ ...agent, state: s }, s.position_x, s.position_y, siteName, siteReason, tick, day);
                if (site) {
            ...
            case 'pilgrimage': {
                ...
                if (resolvedSiteId) {
                    const result = await this.beliefEngine.runPilgrimage({ ...agent, state: s }, resolvedSiteId, tick);
                    if (result) {
            ...
            case 'denounce': {
                ...
                const denounceInfo = perception.nearby_agents.find(a => a.name.toLowerCase() === denounceTargetName?.toLowerCase());
                if (denounceInfo) {
                    await this.beliefEngine.denounce({ ...agent, state: s }, denounceInfo.agent_id, agent.world_id, tick, day);
    ...
    async killAgent(agent, tick, day, state) {
        await (0, db_js_1.execute)(`UPDATE agents.agents SET status = 'dead', death_tick = $1 WHERE agent_id = $2`, [tick, agent.agent_id]);
        ...
        if (agent.created_by_user_id) {
            await (0, db_js_1.execute)(`UPDATE auth.users SET agents_alive_count = GREATEST(0, agents_alive_count - 1)
         WHERE user_id = $1`, [agent.created_by_user_id]);
        }
        console.log(`[AgentEngine] ${agent.name} died from ${cause} at tick ${tick}`);
    }
    async loadAgent(agentId) {
        const agentRow = await (0, db_js_1.queryOne)(`SELECT a.*, s.hp, s.position_x, s.position_y,
       ...
       FROM agents.agents a
       JOIN agents.agent_state s ON a.agent_id = s.agent_id
       WHERE a.agent_id = $1 AND a.status = 'alive'`, [agentId]);
        if (!agentRow)
            return null;
        const traits = await (0, db_js_1.queryOne)('SELECT * FROM agents.agent_traits WHERE agent_id = $1', [agentId]);
        const skills = await (0, db_js_1.query)('SELECT skill_name, level, xp FROM agents.agent_skills WHERE agent_id = $1', [agentId]);
        const inventory = await (0, db_js_1.query)('SELECT resource_type, amount, quality FROM economy.inventory WHERE agent_id = $1 AND amount > 0', [agentId]);
        return {
    ...
    async saveState(agentId, state, tick) {
        await (0, db_js_1.execute)(`UPDATE agents.agent_state SET
    ...
    async recordMemory(agentId, tick, day, decision, state) {
            ...
            : state.mental_state === 'stressed' ? 0.6
                : decision.action.startsWith('talk') ? 0.7 // conversations are memorable
                    : decision.action.startsWith('offer_trade') ? 0.65
                        : 0.3;
        await (0, db_js_1.execute)(`INSERT INTO memory.episodic_memories
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7)`, [
        ...
        await (0, db_js_1.execute)(`DELETE FROM memory.episodic_memories
    ...
    async getRelationships(agentId) {
        return (0, db_js_1.query)(`SELECT r.*, a.name as other_agent_name
       FROM social.relationships r
       JOIN agents.agents a ON a.agent_id = r.other_agent_id
       WHERE r.agent_id = $1`, [agentId]);
    }
    async addToInventory(agentId, resourceType, amount) {
       ...
       DO UPDATE SET amount = economy.inventory.amount + $3`, [agentId, resourceType, amount]);
    }
    async publishMoved(agentId, x, y) {
        await this.redis.publish('genesis:events', JSON.stringify({
            type: 'agent:moved',
            agent_id: agentId,
            x,
    ...
    async publishUpdate(agentId, state, decision) {
        await this.redis.publish('genesis:events', JSON.stringify({
