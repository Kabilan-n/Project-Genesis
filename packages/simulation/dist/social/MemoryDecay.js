...
class MemoryDecay {
     ...
     */
    async runDecayPass(worldId, currentTick) {
     ...
     */
    async decayStrength(worldId, currentTick) {
        await (0, db_js_1.execute)(`UPDATE memory.episodic_memories m
     ...
     */
    async consolidateWeakMemories(worldId, currentTick) {
        // Get all living agents in this world
        const agents = await (0, db_js_1.query)(`SELECT agent_id, name FROM agents.agents
       WHERE world_id = $1 AND status = 'alive'`, [worldId]);
        for (const agent of agents) {
            await this.consolidateForAgent(agent.agent_id, agent.name, currentTick);
        }
    }
    async consolidateForAgent(agentId, agentName, currentTick) {
        // Find weak memories eligible for consolidation
        const weakMemories = await (0, db_js_1.query)(`SELECT memory_id, summary, tick, importance, emotional_valence
       FROM memory.episodic_memories
       WHERE agent_id = $1
         AND current_strength < $2
         AND importance < 0.5
       ORDER BY tick ASC
       LIMIT $3`, [agentId, CONSOLIDATION_THRESHOLD, CONSOLIDATION_BATCH * 3]);
    ...
    async consolidateBatch(agentId, agentName, memories, currentTick) {
...
Reply with ONLY the summary sentence, nothing else.`;
        ...
        await (0, db_js_1.execute)(`DELETE FROM memory.episodic_memories WHERE memory_id = ANY($1)`, [ids]);
