...
exports.civilisationRoutes = civilisationRoutes;
const db_js_1 = require("../db.js");
async function civilisationRoutes(app) {
    // ── Phase 5: Conflict ──────────────────────────────────────────────────────
    // All wars in a world
    app.get('/worlds/:id/wars', async (req) => {
        ...
        if (status)
            params.push(status);
        return (0, db_js_1.query)(`SELECT w.*,
       ...
       ORDER BY w.declared_tick DESC`, params);
    });
    // Single war detail + skirmishes
    app.get('/wars/:id', async (req, reply) => {
        const { id } = req.params;
        const war = await (0, db_js_1.queryOne)(`SELECT w.*,
              ag.name as aggressor_name, ag.colour as aggressor_colour,
              dg.name as defender_name,  dg.colour as defender_colour
       FROM conflict.wars w
       JOIN social.groups ag ON ag.group_id = w.aggressor_group_id
       ...
       WHERE w.war_id = $1`, [id]);
        if (!war)
            return reply.code(404).send({ error: 'War not found' });
        const skirmishes = await (0, db_js_1.query)(`SELECT s.*,
    ...
    app.get('/worlds/:id/skirmishes', async (req) => {
        ...
        return (0, db_js_1.query)(`SELECT s.*,
              at.name as attacker_name, dt.name as defender_name,
    ...
    app.get('/worlds/:id/treaties', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT t.*,
       ...
       ORDER BY t.ratified_tick DESC`, [id]);
    });
    ...
    app.get('/worlds/:id/currencies', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT c.*, g.name as group_name
       FROM civilisation.currencies c
       LEFT JOIN social.groups g ON g.group_id = c.group_id
       WHERE c.world_id = $1
       ORDER BY c.created_tick ASC`, [id]);
    });
    // Agent wallet balances
    app.get('/agents/:id/wallets', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT w.balance, w.total_earned, w.total_spent,
    ...
    app.get('/worlds/:id/technologies', async (req) => {
       ...
       WHERE t.world_id = $1
       ORDER BY t.discovered_tick ASC`, [id]);
    });
    // Agent's known technologies
    app.get('/agents/:id/technologies', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT at.learned_tick, at.learned_from_agent_id,
       ...
       WHERE at.agent_id = $1
       ORDER BY at.learned_tick DESC`, [id]);
    });
    ...
    app.get('/worlds/:id/structures', async (req) => {
       ...
       ORDER BY s.built_tick DESC`, [id]);
    });
    // ── Phase 5: Chronicle ─────────────────────────────────────────────────────
    // Historical chronicle entries for a world
    app.get('/worlds/:id/chronicles', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT chronicle_id, era_name, era_start_day, era_end_day,
              narrative, key_events, generated_tick, created_at
       FROM civilisation.chronicles
       WHERE world_id = $1
       ORDER BY era_start_day ASC`, [id]);
    });
    // Single chronicle entry
    app.get('/chronicles/:id', async (req, reply) => {
        const { id } = req.params;
        const chronicle = await (0, db_js_1.queryOne)(`SELECT * FROM civilisation.chronicles WHERE chronicle_id = $1`, [id]);
        if (!chronicle)
            return reply.code(404).send({ error: 'Chronicle not found' });
        return chronicle;
    });
    // ── Phase 6: Beliefs ──────────────────────────────────────────────────────
    ...
    app.get('/worlds/:id/beliefs', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT b.*, fa.name as founder_name
       FROM culture.belief_systems b
       LEFT JOIN agents.agents fa ON fa.agent_id = b.founder_id
       WHERE b.world_id = $1
       ORDER BY b.adherent_count DESC`, [id]);
    });
    // Single belief system detail + adherents
    app.get('/beliefs/:id', async (req, reply) => {
        const { id } = req.params;
        const belief = await (0, db_js_1.queryOne)(`SELECT b.*, fa.name as founder_name
       FROM culture.belief_systems b
       LEFT JOIN agents.agents fa ON fa.agent_id = b.founder_id
       WHERE b.belief_id = $1`, [id]);
        if (!belief)
            return reply.code(404).send({ error: 'Belief not found' });
        const adherents = await (0, db_js_1.query)(`SELECT ab.conviction, ab.adopted_tick, ab.adopted_day,
       ...
       ORDER BY ab.conviction DESC`, [id]);
        const myths = await (0, db_js_1.query)(`SELECT m.myth_id, m.title, m.narrative, m.spread_count,
              m.believability, m.created_tick, m.created_day,
              a.name as author_name
       FROM culture.myths m
       JOIN agents.agents a ON a.agent_id = m.author_id
       WHERE m.belief_id = $1
       ORDER BY m.spread_count DESC`, [id]);
        return { ...belief, adherents, myths };
    });
    // Agent's beliefs and conviction levels
    app.get('/agents/:id/beliefs', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT ab.conviction, ab.personal_interpretation, ab.adopted_tick, ab.adopted_day,
    ...
    app.get('/worlds/:id/myths', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT m.*,
              a.name as author_name,
       ...
       WHERE m.world_id = $1
       ORDER BY m.spread_count DESC, m.created_tick DESC`, [id]);
    });
    // Agent's known myths
    app.get('/agents/:id/myths', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT am.fidelity, am.local_version, am.learned_tick,
    ...
    app.get('/worlds/:id/rituals', async (req) => {
       ...
       WHERE r.world_id = $1
       ORDER BY r.times_performed DESC`, [id]);
    });
    ...
    app.get('/worlds/:id/snapshots', async (req) => {
        ...
        return (0, db_js_1.query)(`SELECT snapshot_id, tick, day, time_of_day, agent_count,
    ...
    app.get('/snapshots/:id', async (req, reply) => {
        const { id } = req.params;
        const snap = await (0, db_js_1.queryOne)(`SELECT * FROM observer.world_snapshots WHERE snapshot_id = $1`, [id]);
        if (!snap)
    ...
    app.get('/worlds/:id/heatmap', async (req) => {
        const { id } = req.params;
        const { day, type = 'population' } = req.query;
        const latestDay = day
            ? parseInt(day)
            : (await (0, db_js_1.queryOne)(`SELECT MAX(day) as max_day FROM observer.heatmap_data WHERE world_id = $1`, [id]))?.max_day ?? 0;
        const col = {
        ...
        return (0, db_js_1.query)(`SELECT x, y, ${col} as value
       FROM observer.heatmap_data
       WHERE world_id = $1 AND day = $2 AND ${col} > 0`, [id, latestDay]);
    });
    // Agent biography (cached LLM narrative)
    app.get('/agents/:id/biography', async (req, reply) => {
        const { id } = req.params;
        const bio = await (0, db_js_1.queryOne)(`SELECT * FROM observer.agent_biographies
       WHERE agent_id = $1
       ORDER BY generated_tick DESC LIMIT 1`, [id]);
        if (!bio)
            return reply.code(404).send({ error: 'No biography generated yet' });
        return bio;
    });
    ...
    app.get('/agents/:id/family', async (req) => {
        ...
        const allAgents = await (0, db_js_1.query)(`SELECT agent_id, name, archetype, generation, status,
              parent_agent_ids, birth_tick, death_tick
       FROM agents.agents
       WHERE world_id = $1
       ORDER BY generation ASC, birth_tick ASC`, [agent.world_id]);
        // Build ancestry subtree: ancestors of the target agent up to 3 levels
        const agentMap = new Map(allAgents.map((a) => [a.agent_id, a]));
        function buildAncestry(agentId, depth) {
            ...
            return {
                ...
                parents: depth > 0
                    ? a.parent_agent_ids
                        .map((pid) => buildAncestry(pid, depth - 1))
                        .filter(Boolean)
                    : [],
            };
        }
        function buildDescendants(agentId, depth) {
            const a = agentMap.get(agentId);
            if (!a)
                return null;
            const children = allAgents.filter((x) => x.parent_agent_ids.includes(agentId));
            return {
                agent_id: a.agent_id,
                name: a.name,
                archetype: a.archetype,
                ...
                children: depth > 0
                    ? children
                        .map((c) => buildDescendants(c.agent_id, depth - 1))
                        .filter(Boolean)
                    : [],
            };
        }
        ...
        const siblings = focal?.parent_agent_ids?.length
            ? allAgents.filter((a) => a.agent_id !== id &&
                a.parent_agent_ids.some((pid) => focal.parent_agent_ids.includes(pid))).map((a) => ({ agent_id: a.agent_id, name: a.name, archetype: a.archetype, generation: a.generation, status: a.status }))
            : [];
    ...
    app.get('/worlds/:id/reproductions', async (req) => {
        const { id } = req.params;
        const { limit = '20', offset = '0' } = req.query;
        return (0, db_js_1.query)(`SELECT rr.*,
    ...
    app.get('/agents/:id/trait-drift', async (req) => {
        const { id } = req.params;
        return (0, db_js_1.query)(`SELECT * FROM agents.trait_drift_events
       WHERE agent_id = $1
       ORDER BY tick DESC
       LIMIT 30`, [id]);
    });
