...
const SOUL_TRAITS: Record<string, number>[][] = [
...
export async function agentRoutes(app: FastifyInstance) {
  ...
  app.post('/agents/create', async (req, reply) => {
    const { world_id, name, answers } = req.body as {
      world_id: string;
      name: string;
    ...
    // Insert agent
    await execute(
      `INSERT INTO agents.agents (agent_id, world_id, name, archetype, birth_tick)
       VALUES ($1,$2,$3,$4,$5)`,
      [agentId, world_id, name.trim(), archetype, birthTick]
    );
  ...
  app.get('/agents/:id', async (req, reply) => {
    ...
    const agent = await queryOne<any>(
      `SELECT a.agent_id, a.world_id, a.name, a.archetype, a.status,
    ...
    const memories = await query(
      `SELECT memory_id, tick, day, summary, emotional_valence, emotional_intensity, importance
  ...
  app.get('/agents/:id/relationships', async (req) => {
    const { id } = req.params as { id: string };
    return query<any>(
      `SELECT r.other_agent_id, a.name as other_name, a.archetype as other_archetype,
  ...
  app.get('/agents/:id/memories', async (req) => {
    ...
    return query(
      `SELECT * FROM memory.episodic_memories
       WHERE agent_id = $1
       ORDER BY importance DESC, tick DESC
       LIMIT $2`,
      [id, parseInt(limit)]
  ...
  app.get('/agents/:id/history', async (req) => {
    const { id } = req.params as { id: string };
    return query(
      `SELECT event_id, tick, day, event_type, significance, title, summary, created_at
       FROM events.events
  ...
  app.get('/agents/:id/thoughts', async (req, reply) => {
    ...
    const state = await queryOne<any>(
      `SELECT s.current_goal, s.mental_state, s.current_activity,
