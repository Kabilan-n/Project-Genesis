...
function calculateArchetype(traits: Record<string, number>): string {
  const get = (k: string) => traits[k] ?? 50;

  // First word: approach to life
  let firstWord = 'Thoughtful';
  if (get('optimism') > 60) firstWord = 'Hopeful';
  else if (get('optimism') < 40) firstWord = 'Cautious';
  else if (get('impulsivity') > 60) firstWord = 'Spontaneous';
...
export async function onboardingRoutes(app: FastifyInstance) {
  ...
  app.post('/onboarding/sessions', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    await execute(
      `INSERT INTO auth.onboarding_sessions (session_id, user_id, world_id, mode)
       VALUES ($1,$2,$3,$4)`,
      [sessionId, user_id, world_id ?? null, mode]
    );
  ...
  app.post('/onboarding/sessions/:id/answers', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    const { question_number, answer_index } = req.body as {
      question_number: number; answer_index: number;
    };

    const session = await queryOne<{ session_id: string; answers: any[]; accumulated_traits: any }>(
      'SELECT session_id, answers, accumulated_traits FROM auth.onboarding_sessions WHERE session_id = $1',
      [id]
    );
    if (!session) return reply.code(404).send({ error: 'Session not found' });

    const question = SOUL_QUESTIONS[question_number - 1];
    if (!question) return reply.code(400).send({ error: 'Invalid question number' });

    const option = question.options[answer_index];
    if (!option) return reply.code(400).send({ error: 'Invalid answer index' });
    ...
    await execute(
      `UPDATE auth.onboarding_sessions
       ...
       WHERE session_id = $3`,
      [JSON.stringify(answers), JSON.stringify(traits), id]
    );
  ...
  app.post('/onboarding/sessions/:id/identity', { onRequest: [app.authenticate] }, async (req, reply) => {
    const { id } = req.params as { id: string };
    const { name, appearance = {} } = req.body as { name: string; appearance?: Record<string, unknown> };
  ...
  app.post('/onboarding/sessions/:id/complete', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    const session = await queryOne<{
      session_id: string; user_id: string; world_id: string | null;
      accumulated_traits: any; name: string | null; appearance: any;
    }>(
      'SELECT * FROM auth.onboarding_sessions WHERE session_id = $1 AND status = $2',
      [id, 'in_progress']
    );

    if (!session) return reply.code(404).send({ error: 'Session not found or already completed' });
    if (!session.name) return reply.code(400).send({ error: 'Must set identity before completing' });

    // Get world_id — use provided or pick first active world
    let worldId = session.world_id;
    if (!worldId) {
      const world = await queryOne<{ world_id: string }>(
        `SELECT world_id FROM worlds.worlds WHERE status = 'active' LIMIT 1`
      );
      if (!world) return reply.code(400).send({ error: 'No active world available' });
    ...
    // Find spawn point
    const spawnTile = await queryOne<{ x: number; y: number }>(
      `SELECT x, y FROM worlds.map_tiles
    ...
    // Get current tick
    const world = await queryOne<{ current_tick: number }>(
    ...
    // Insert agent
    await execute(
      `INSERT INTO agents.agents (agent_id, world_id, created_by_user_id, name, archetype, birth_tick)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [agentId, worldId, user_id, session.name, archetype, birthTick]
    );

    // Insert traits (defaults + answers accumulated)
    const t = (key: string, def = 50) => Math.max(0, Math.min(100, Math.round(traits[key] ?? def)));
    await execute(
    ...
    // Insert state
    await execute(
      `INSERT INTO agents.agent_state (agent_id, position_x, position_y)
       VALUES ($1,$2,$3)`,
      [agentId, spawnX, spawnY]
    );
    ...
    // Starting skills
    await execute(
      `INSERT INTO agents.agent_skills (agent_id, skill_name, level) VALUES ($1,'foraging',1)`,
      [agentId]
    );

    // Mark session complete + update user counts
    await execute(
      `UPDATE auth.onboarding_sessions SET status = 'completed' WHERE session_id = $1`,
      [id]
    );
    await execute(
      `UPDATE auth.users
