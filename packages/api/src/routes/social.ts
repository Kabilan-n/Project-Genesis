...
export async function socialRoutes(app: FastifyInstance) {
  ...
  app.get('/worlds/:id/trades', async (req) => {
    ...
    const { limit = '20', offset = '0' } = req.query as { limit?: string; offset?: string };
    return query<any>(
      `SELECT t.trade_id, t.tick, t.day, t.status, t.outcome_reason, t.created_at,
  ...
  app.get('/agents/:id/trades', async (req) => {
    ...
    const { limit = '10' } = req.query as { limit?: string };
    return query<any>(
      `SELECT t.trade_id, t.tick, t.day, t.status, t.offered_items, t.requested_items,
              oa.name as offerer_name, ra.name as receiver_name
       FROM economy.trades t
       JOIN agents.agents oa ON oa.agent_id = t.offerer_agent_id
       JOIN agents.agents ra ON ra.agent_id = t.receiver_agent_id
  ...
  app.get('/groups/:id', async (req, reply) => {
    ...
    const members = await query<any>(
      `SELECT gm.role, gm.joined_tick, gm.joined_day, gm.contribution_score,
  ...
  app.get('/agents/:id/group', async (req, reply) => {
    ...
    if (!row) return reply.code(404).send({ error: 'Agent is not in a group' });
    return row;
  });
  ...
  app.get('/agents/:id/reputation', async (req, reply) => {
    ...
    const agent = await queryOne<{ world_id: string }>(
      `SELECT world_id FROM agents.agents WHERE agent_id = $1`,
      [id]
    );
  ...
  app.get('/agents/:id/knowledge', async (req) => {
    ...
    return query<any>(
      `SELECT k.*,
       ...
       WHERE k.agent_id = $1
       ORDER BY k.confidence DESC, k.times_shared DESC`,
      [id]
    );
  });
